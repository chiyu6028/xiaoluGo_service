...
public class RedisTest {
    ...
    public void before(){
        ...
        redisson = (RedissonClient) cx.getBean("redisson");
    }

    @Test
    public void testGetRBucket() {
        RBucket<String> rBucket= RedisUtil.getInstance().getRBucket(redisson, "testBucket");
        //同步放置
    ...
    public void testGetRMap() throws InterruptedException, ExecutionException {
        RMap<String, Integer> rMap=RedisUtil.getInstance().getRMap(redisson, "testMap");
        //清除集合
        rMap.clear();
    ...
    public void testGetRSortedSet() {
        RSortedSet<Integer> rSortedSet=RedisUtil.getInstance().getRSortedSet(redisson, "testSortedSet");
        //清除集合
        rSortedSet.clear();
        rSortedSet.add(45);
        rSortedSet.add(12);
    ...
    public void testGetRSet() {
        RSet<Integer> rSet=RedisUtil.getInstance().getRSet(redisson, "testSet");
        //清除集合
        rSet.clear();
    ...
    public void testGetRList() {
        RList<Integer> rList=RedisUtil.getInstance().getRList(redisson, "testList");
        //清除集合
        rList.clear();
        Collection<Integer> c=Arrays.asList(12,45,12,34,56,78);
        rList.addAll(c);
    ...
    public void testGetRQueue() {
        RQueue<Integer> rQueue=RedisUtil.getInstance().getRQueue(redisson, "testQueue");
        //清除队列
        rQueue.clear();
        Collection<Integer> c=Arrays.asList(12,45,12,34,56,78);
        rQueue.addAll(c);
    ...
    public void testGetRDeque() {
        RDeque<Integer> rDeque=RedisUtil.getInstance().getRDeque(redisson, "testDeque");
        //清空双端队列
        rDeque.clear();
        Collection<Integer> c=Arrays.asList(12,45,12,34,56,78);
        rDeque.addAll(c);
    ...
    public void testGetRLock() {
        RLock rLock=RedisUtil.getInstance().getRLock(redisson, "testLock1");
    ...
    public void testGetRAtomicLong() {
        RAtomicLong rAtomicLong=RedisUtil.getInstance().getRAtomicLong(redisson, "testAtomicLong");
        rAtomicLong.set(100);
    ...
    public void testGetRCountDownLatch() throws InterruptedException {
        RCountDownLatch rCountDownLatch=RedisUtil.getInstance().getRCountDownLatch(redisson, "testCountDownLatch");
    ...
    public void testGetRTopicSub() throws InterruptedException {
        RTopic<String> rTopic=RedisUtil.getInstance().getRTopic(redisson, "testTopic");
        rTopic.addListener(new MessageListener<String>() {

            @Override
            public void onMessage(String msg,String str) {
                // TODO Auto-generated method stub
                System.out.println("你发布的是:"+msg+":"+str);
            }
        });
        //等待发布者发布消息
        RCountDownLatch rCountDownLatch=RedisUtil.getInstance().getRCountDownLatch(redisson, "testCountDownLatch");
